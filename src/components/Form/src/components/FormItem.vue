  ...
  export default defineComponent({
    ...
    setup(props, { slots }) {
      ...
      const getComponentsProps = computed(() => {
        const { schema, tableAction, formModel, formActionType, formCacheData } = props;
        let { componentProps = {} } = schema;
        if (isFunction(componentProps)) {
          componentProps = componentProps({ schema, tableAction, formModel, formActionType, formCacheData }) ?? {};
        }
      ...
      return () => {
        ...
        const realColProps = { span: 24, ...baseColProps, ...colProps };
